...
#else
#include "lpbsp.h"
#define MZ_MALLOC(x) lpbsp_malloc(x)
#define MZ_FREE(x) lpbsp_free(x)
#define MZ_REALLOC(p, x) lpbsp_realloc(p, x)
#endif
...
enum
{
    ...
    // 设置zip一次性读取文件的大小-esp32 spi dma读取需要内部ram
    MZ_ZIP_MAX_IO_BUF_SIZE = 0x4000,
